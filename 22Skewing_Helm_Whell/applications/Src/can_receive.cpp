 ...
 * @note        无
 */
void CAN_BUS::CMD::CAN1_Front_gm3508(int16_t motor1, int16_t motor2, int16_t motor3, int16_t motor4)
{
...
/**
 * @brief       CAN1发送函数(后4个电机)此工程为控制6020电机函数 
 ...
 * @note        无
 */
 void CAN_BUS::CMD::CAN1_Behind_gm6020(int16_t motor5, int16_t motor6, int16_t motor7, int16_t motor8)
{
 uint8_t	can_send_data[8];
 uint32_t send_mail_box;
 CAN_TxHeaderTypeDef can_tx_message;
 ...
 can_send_data[0] = (motor5 >> 8) & 0xff;
 can_send_data[1] = (motor5) & 0xff;
 can_send_data[2] = (motor6 >> 8) & 0xff;
 can_send_data[3] = (motor6) & 0xff;
 can_send_data[4] = (motor7 >> 8) & 0xff;
 can_send_data[5] = (motor7) & 0xff;
 can_send_data[6] = (motor8 >> 8) & 0xff;
 can_send_data[7] = (motor8) & 0xff;
 HAL_CAN_AddTxMessage(&hcan1, &can_tx_message, can_send_data, &send_mail_box);
}
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
	...
	// 	if(rx_header.StdId<CAN_6020_M1_ID)
	// 	{
	...
	// 	}
	// 	//6020
	// 	if((rx_header.StdId>=CAN_6020_M1_ID)&&(rx_header.StdId<CAN_6020_M1_ID+MOTOR_MAX_NUM))
	// 	{
	// 		static uint8_t j;
	// 		j = rx_header.StdId-CAN_6020_M1_ID;
	// 		// can_bus.motor_6020[j].angle         = ((rx_data_1[0] << 8) | rx_data_1[1]);
	// 		// can_bus.motor_6020[j].speed_rpm     = ((rx_data_1[2] << 8) | rx_data_1[3]);
	// 		// can_bus.motor_6020[j].given_current = ((rx_data_1[4] << 8) | rx_data_1[5]);
	// 		// can_bus.motor_6020[j].temperate     =  rx_data_1[6];

	// 		can_bus.motor_6020[j].msg_cnt++ <= 50 ? can_bus.dji_encoder.get_moto_offset(&can_bus.motor_6020[j], rx_data_2) : can_bus.dji_encoder.get_motor_measure(&can_bus.motor_6020[j], rx_data_2);
	// 		can_bus.dji_encoder.get_motor_measure(&can_bus.motor_6020[j], rx_data_2);
	...
	{
		HAL_CAN_GetRxMessage(&hcan2, CAN_RX_FIFO0, &rx_header, rx_data_2);
		//3508
		if(rx_header.StdId<CAN_6020_M1_ID)
		{
			switch (rx_header.StdId)
...
//				((ptr)->angle) = (int32_t)(((ptr)->ecd) - ((ptr)->last_ecd));
					...
					if(ptr->angle - ptr->last_angle > 4096)
						...
						ptr->round_cnt ++;
					ptr->total_angle = ptr->round_cnt * 8192 + ptr->angle - ptr->offset_angle+ptr->init_angle;//+ptr->init_angle
