...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
	...
	if(hcan->Instance==CAN1)
	{
		...
		if((rx_header.StdId>=CAN_6020_M1_ID)&&(rx_header.StdId<CAN_6020_M1_ID+MOTOR_MAX_NUM))
		{
			...
			can_bus.motor_6020[j].msg_cnt++ <= 50 ? can_bus.dji_encoder.get_moto_offset(&can_bus.motor_6020[j], rx_data_1) : can_bus.dji_encoder.get_motor_measure(&can_bus.motor_6020[j], rx_data_1);
			can_bus.dji_encoder.get_motor_measure(&can_bus.motor_6020[j], rx_data_1);
