...
void ROBOT::CHASSIC::chassic_Init()
{
    ...
    for(;;)
    {
        motor_traget[0]=pid_controller.can_motor.GM6020_VP_Dual_Loop_Realize(5500, 1);
        motor_traget[1]=pid_controller.can_motor.GM6020_VP_Dual_Loop_Realize(1300, 2);
        motor_traget[2]=pid_controller.can_motor.GM6020_VP_Dual_Loop_Realize(7550, 3);

        can_bus.cmd.CAN1_Behind_gm6020(motor_traget[0],motor_traget[1], motor_traget[2], 0);

        HAL_Delay(5);
        if((can_bus.motor_6020[0].angle>5490)&&(can_bus.motor_6020[0].angle<5510)&&(can_bus.motor_6020[2].angle>7540)&&(can_bus.motor_6020[1].angle>=1290)&&(can_bus.motor_6020[1].angle<1315))
        {
            break;
        }
...
void ROBOT::CHASSIC::Helm_Move_Calc(fp32 v_y,fp32 v_x,fp32 v_omega)
{
    ...
    {
        motor_3508[i].speed=std::sqrt((speed_x[i]*speed_x[i])+(speed_y[i]*speed_y[i]));
    }
...
/*计算编码器要偏移的角度*/
    pos_6020[0]=pid_controller.can_motor.GM6020_VP_Dual_Loop_Realize(5500-motor_6020[2].position, 1);
    pos_6020[1]=pid_controller.can_motor.GM6020_VP_Dual_Loop_Realize(1300-motor_6020[0].position, 2);
    pos_6020[2]=pid_controller.can_motor.GM6020_VP_Dual_Loop_Realize(7550-motor_6020[1].position, 3);   
...
//		pid_controller.can_motor.GM6020_VP_Dual_Loop_Realize(4096,1), 
//		0,
//		0, 
//		0);
...
fp32 ROBOT::CHASSIC::Angle_Calc(fp32 vx,fp32 vy)
{
    ...
    if(vx>0&&vy>=0)
    {
        angle=robot.chassic.Angle_To_Encoder(robot.chassic.rad2deg(theta));
    }
    //第二象限
    else if (vx<0&&vy>=0)
    {
    ...
    {
        angle=robot.chassic.Angle_To_Encoder(robot.chassic.rad2deg(theta)+360.0f);
    }
    //第四象限
    else if(vx>0&&vy<0)
    {
