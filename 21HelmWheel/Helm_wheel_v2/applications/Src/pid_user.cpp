...
pid_type_def pid_v2_6020[3],pid_pos2_6020[3];
pid_type_def pid_yaw;
//电机PID三参
fp32 motor_speed_3508_pid[3] = {10,0,5};//底盘3508参数
fp32 motor_position_3508_pid[3] = {1,0.01,1};

fp32 motor_speed_6020_pid[3]={40,0.1,5};		//底盘6020参数绝对值
fp32 motor_position_6020_pid[3]={1,0,1};

fp32 motor_speed_6020_pid_2[3]={40,0.1,5};		//底盘6020参数相对值
fp32 motor_position_6020_pid_2[3]={3,0,1};


fp32 motor_yaw_pid[3] = {25,25/200,1};//yaw轴pid参数
...
void PID_Controller::All_Device_Init(void)
{
	...
	for(int j = 0;j<4;j++)	
	{
	this->core.PID_Init(&pid_v_6020[j], PID_POSITION, motor_speed_6020_pid, 20000, 10000);
	this->core.PID_Init(&pid_pos_6020[j], PID_POSITION, motor_position_6020_pid, 200, 10);
	
   this->core.PID_Init(&pid_v2_6020[j], PID_POSITION, motor_speed_6020_pid_2, 20000, 500);
   this->core.PID_Init(&pid_pos2_6020[j], PID_POSITION, motor_position_6020_pid_2, 400, 10);
	}

	for(int i=0;i<4;i++)
	{	
    this->core.PID_Init(&pid_v_1[i], PID_POSITION, motor_speed_3508_pid, 10000, 5000);
	this->core.PID_Init(&pid_pos_1[i], PID_POSITION, motor_position_3508_pid, 10000, 300);
	}


	this->core.PID_Init(&pid_yaw, PID_POSITION, motor_yaw_pid,10000,500);
}
...
fp32 PID_Controller::CAN_MOTOR::GM6020_Velocity_Realize_2(fp32 set_speed,int i)
{
	pid_controller.core.PID_Calc(&pid_v2_6020[i-1], can_bus.motor_6020[i-1].speed_rpm,set_speed);
	return pid_v2_6020[i-1].out;
}
fp32 PID_Controller::CAN_MOTOR::GM6020_Position_Realize_2(fp32 set_pos,int i)
{
	pid_controller.core.PID_Calc(&pid_pos2_6020[i-1], can_bus.motor_6020[i-1].total_angle, set_pos);
	return pid_pos2_6020[i-1].out;
}

fp32 PID_Controller::CAN_MOTOR::GM6020_VP_Dual_Loop_Realize_2(fp32 set_pos,int i)
{
 return GM6020_Velocity_Realize(GM6020_Position_Realize_2(set_pos,  i), i);
}
/**
 * @brief       yaw轴pid
