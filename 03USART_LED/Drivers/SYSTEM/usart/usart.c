...
void HAL_UART_MspInit(UART_HandleTypeDef *huart)
{
    ...
    {
        __HAL_RCC_USART1_CLK_ENABLE();                          /*使能中断时钟*/
        __HAL_RCC_GPIOA_CLK_ENABLE();                           /*使能GPIO时钟*/

    //初始化引脚PA9（发送TX）
        gpio_init_struct1.Pin = GPIO_PIN_9;                      /* 串口发送引脚号 */
        gpio_init_struct1.Mode = GPIO_MODE_AF_PP;                /* 复用推挽输出 （输出高低电平）*/
        gpio_init_struct1.Pull = GPIO_PULLUP;                    /* 上拉 */
        gpio_init_struct1.Speed = GPIO_SPEED_FREQ_HIGH;          /* IO速度设置为高速 */
        HAL_GPIO_Init(GPIOA, &gpio_init_struct1);

    //初始化引脚PA10（接收RX）
        gpio_init_struct1.Pin = GPIO_PIN_10;                      /* 串口接收引脚号 */
        gpio_init_struct1.Mode = GPIO_MODE_AF_INPUT;              /* 输入*/
        gpio_init_struct1.Pull = GPIO_PULLUP;                     /* 上拉 */
        gpio_init_struct1.Speed = GPIO_SPEED_FREQ_HIGH;           /* IO速度设置为高速 */
        HAL_GPIO_Init(GPIOA, &gpio_init_struct1);
...
void USART1_IRQHandler (void)
{
    ...
    HAL_UART_Receive_IT(&g_uart1_handle, g_rx_buffer, 1);           /*因为上面的函数失能了，所以重新调用中断*/
}
