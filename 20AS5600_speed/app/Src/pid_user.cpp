...
//电机PID三参
fp32 motor_speed_3508_pid[3] = {60, 0 , 3};//底盘3508参数
fp32 motor_position_3508_pid[3] = {30, 0, 2};
fp32 motor_speed_2006_pid[3] = {10,0,0.1};//底盘2006参数
...
void PID_Controller::All_Device_Init(void)
{
	...
	for(int i=0;i<4;i++)
	{
    	this->core.PID_Init(&pid_v_1[i], PID_POSITION, motor_speed_3508_pid, 20000, 6000);
		this->core.PID_Init(&pid_pos_1[i], PID_POSITION, motor_position_3508_pid, 3000, 300);
		
		this->core.PID_Init(&pid_v_1[i], PID_POSITION, motor_speed_2006_pid, 20000, 6000);
		this->core.PID_Init(&pid_pos_1[i], PID_POSITION, motor_position_2006_pid, 3000, 300);
